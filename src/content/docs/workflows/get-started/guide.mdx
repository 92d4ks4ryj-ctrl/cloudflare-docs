...
2. Has at least one or more calls to `step.do` that define the execution of your workflow.
...
Each `step` in a Workflow is an independently retriable function. Steps can return (optional) state, allowing a Workflow to continue execution even if subsequent steps fail, without having to re-run all previous steps.


A `step` is what makes a Workflow powerful, as you can encapsulate errors and persist state as your Workflow progresses from step to step.

* A step can execute code (`step.do`), wait for external input (`step.waitForEvent`), or sleep a Workflow (`step.sleep`).
...
Return to the `src/index.ts` file we created in the previous step and add a `fetch` handler that _binds_ to our Workflow. This binding allows us to create new Workflow instances [^1] and take action upon them, including pausing, terminating, and fetching their status.

[^1]: Each execution of a Workflow is referred to as an instance. 
...
Because Workflows can be long running, it is possible to have running instances that represent different versions of your Workflow code over time. Once the instance has started, it will execute steps as defined in the version of the workflow that existed at its point of creation. To ensure an instance is executing the most recent version of a workflow, it must be restarted.
...
You can also trigger a Workflow by navigating in the Cloudflare Dashboard to **Workflows** -> Select an instance -> **Trigger**. 
