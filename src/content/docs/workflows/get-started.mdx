...
import { Details, LinkCard, Render, PackageManagers, WranglerConfig } from "~/components"
...
Open a terminal and run C3 to create your Worker project:
...
A Workflow extends `WorkflowEntrypoint` and implements a `run` method. Inside your Workflow, you can access [bindings](/workers/runtime-apis/bindings/) (such as [KV](/kv/), [R2](/r2/), or [D1](/d1/)) via `this.env`.
...
Replace `src/index.ts` with a fetch handler to start and check Workflow instances:
...
The `MyWorkflow` class must be re-exported from `index.ts` so Wrangler can find it.

## 4. Configure your Workflow

Open `wrangler.jsonc` and add the `workflows` configuration:
...
Test in production using the same curl commands against your deployed URL.

You can also inspect Workflow instances with the CLI:
